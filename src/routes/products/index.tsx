    ...
    <PageLayout>
      <main className="pt-24 sm:pt-28 lg:pt-32 pb-24 text-start">
        <Container clean className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
          <PageHeader title={t("products.headerTitle")} description={t("products.headerDescription")} />

          {isLoading && <StateFeedback type="loading" />}
          {isError && <StateFeedback type="error" message={t("products.loadError")} onRetry={refetch} />}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-6">
                      ...
                      {product.badge && (
                        <span className={`px-3 py-1 rounded-full font-label-md text-label-md ${isSpecialized ? "bg-tertiary-fixed text-on-tertiary-fixed" : "bg-secondary-container text-on-secondary-container"}`}>
                          {product.badge}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="space-y-3 mb-8">
                      {product.features.map((feature, fIdx) => (
                        <div key={fIdx} className="flex items-center gap-2 text-secondary justify-start">
                          <span className="material-symbols-outlined text-sm" aria-hidden="true">check_circle</span>
                          <span className="font-label-md text-label-md text-on-surface">{feature}</span>
                        </div>
                      ))}
                    </div>

                    <div className="flex flex-col gap-3">
                      <Link to="/quote" search={{ product: product.slug, source: "products-listing" }}
                      ...
                      <Link to="/consultation" search={{ source: "products-listing" }}
                        ...
                        {product.ctaSecondary || (locale === "ar" ? "تخصيص لعملك" : "Customize for you")}
          ...
          <section className="mt-24 bg-primary-container p-12 rounded-xl text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="font-headline-xl text-headline-xl text-on-primary mb-6 font-bold">{t("products.customCtaTitle")}</h2>
              <p className="font-body-lg text-body-lg text-on-primary-container max-w-2xl mx-auto mb-10 opacity-90">{t("products.customCtaSubtitle")}</p>
              <Link to="/solutions" search={{ source: "products-custom-cta" }}
