      ...
      <main className="pt-24 sm:pt-28 lg:pt-32 pb-24 text-start bg-background">
        <Container clean className="max-w-container-max mx-auto px-margin-desktop mb-12">
          ...
          <div className="text-right">
            <h1 className="font-display-lg text-display-lg text-primary mb-6 font-bold">{t("careers.jobsTitle")}</h1>
            <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl">{t("careers.jobsSubtitle")}</p>
          </div>
        </Container>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter text-right">
                {careers.map((job, index) => {
                  ...
                  if (isFeatured) return (
                    <div key={job.id} className="md:col-span-2 group relative overflow-hidden bg-surface-container-lowest p-8 rounded-2xl border border-outline-variant/30 hover:shadow-xl hover:border-secondary/50 transition-all duration-300">
                      <div className="absolute top-0 right-0 w-32 h-32 bg-secondary/5 rounded-full -translate-y-16 translate-x-16" aria-hidden="true" />
                      <div className="flex justify-between items-start mb-6 relative z-10">
                        ...
                        <div className="material-symbols-outlined text-secondary text-4xl" aria-hidden="true">{getJobIcon(job.title)}</div>
                      </div>
                      <Link to="/careers/$slug" params={{ slug: job.slug }} className="block relative z-10">
                        <h3 className="font-headline-lg text-headline-lg text-primary mb-4 font-bold group-hover:text-secondary transition-colors">{job.title}</h3>
                      </Link>
                      <p className="font-body-md text-body-md text-on-surface-variant mb-6 line-clamp-2 relative z-10">{job.description}</p>
                      <div className="flex flex-wrap gap-2 mb-8 justify-start relative z-10">
                        <span className="px-3 py-1 bg-background text-on-surface-variant font-caption text-caption rounded-lg border border-outline-variant/20 flex items-center gap-1">
                          <span className="material-symbols-outlined text-[14px]">work_history</span> {job.experience}
                        </span>
                        <span className="px-3 py-1 bg-background text-on-surface-variant font-caption text-caption rounded-lg border border-outline-variant/20 flex items-center gap-1">
                  ...
                  if (isAI) return (
                    <div key={job.id} className="group relative overflow-hidden bg-primary text-on-primary p-8 rounded-2xl border border-primary hover:bg-primary/90 hover:shadow-xl hover:shadow-primary/20 transition-all duration-300 flex flex-col justify-between">
                      ...
                      <div className="relative z-10">
                        <div className="material-symbols-outlined text-secondary text-4xl mb-6" aria-hidden="true">{getJobIcon(job.title)}</div>
                        <Link to="/careers/$slug" params={{ slug: job.slug }} className="block">
                          <h3 className="font-headline-lg text-headline-lg mb-4 text-on-primary font-bold group-hover:text-secondary transition-colors">{job.title}</h3>
                        </Link>
                  ...
                  return (
                    <div key={job.id} className="group bg-surface-container-lowest p-8 rounded-2xl border border-outline-variant/30 hover:border-secondary hover:shadow-lg transition-all duration-300 flex flex-col">
                      <div className="material-symbols-outlined text-secondary text-4xl mb-6" aria-hidden="true">{getJobIcon(job.title)}</div>
                      <Link to="/careers/$slug" params={{ slug: job.slug }} className="block">
                        <h3 className="font-headline-sm text-headline-sm text-primary mb-2 font-bold group-hover:text-secondary transition-colors">{job.title}</h3>
                      </Link>
                      ...
                      <div className="mt-auto">
                        <Link to="/careers/$slug" params={{ slug: job.slug }}
                          ...
                          {locale === "ar" ? "عرض التفاصيل والتقديم" : "View Details & Apply"}{" "}
                          <span className="material-symbols-outlined scale-x-[-1] group-hover:translate-x-[-4px] transition-transform" aria-hidden="true">arrow_forward</span>
                        </Link>
                      </div>
                    </div>
                  );
        ...
        {/* Application Form */}
        <section className="py-24 bg-surface-container-low border-t border-outline-variant/20 mt-12" id="apply-form">
          <Container clean className="max-w-container-max mx-auto px-margin-desktop">
            <div className="grid md:grid-cols-5 gap-16">
              ...
              <div className="md:col-span-3 text-right">
                <form onSubmit={handleFormSubmit} className="bg-white p-10 rounded-2xl border border-outline-variant/30 shadow-xl shadow-primary/5 space-y-6" id="career-form">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input name="name" className="w-full bg-surface-container-lowest border border-outline-variant/30 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all text-right" placeholder={t("careers.namePlaceholder")} type="text" required />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-right font-label-md text-label-md text-on-surface-variant">{t("careers.emailLabel")}</label>
                      <input name="email" className="w-full bg-surface-container-lowest border border-outline-variant/30 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all text-right" placeholder={t("careers.emailPlaceholder")} type="email" required />
                    </div>
                  </div>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="block text-right font-label-md text-label-md text-on-surface-variant">{t("careers.phoneLabel")}</label>
                      <input name="phone" className="w-full bg-surface-container-lowest border border-outline-variant/30 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all text-right" placeholder={t("careers.phonePlaceholder")} type="tel" required />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-right font-label-md text-label-md text-on-surface-variant">{t("careers.positionLabel")}</label>
                      <select name="position" className="w-full bg-surface-container-lowest border border-outline-variant/30 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all text-right">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group border-2 border-dashed border-outline-variant/50 rounded-xl p-8 transition-colors hover:border-secondary/50 hover:bg-secondary/5 bg-surface-container-lowest flex flex-col items-center justify-center gap-2 cursor-pointer">
                      ...
                      <span className="material-symbols-outlined text-secondary text-4xl group-hover:scale-110 transition-transform" aria-hidden="true">cloud_upload</span>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea name="cover_letter" className="w-full bg-surface-container-lowest border border-outline-variant/30 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all text-right" placeholder={t("careers.coverLetterPlaceholder")} rows={4} />
                  </div>
