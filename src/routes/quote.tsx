...
const copy = {
  ar: {
    ...
    subtitle:
      "اختر نوع المشروع، حدّد مستوى الاستثمار، واترك لنا تفاصيل كافية لنجهز لك تصورًا عمليًا.",
  ...
  en: {
    ...
    subtitle:
      "Choose the project type, set the investment range, and share context for a practical execution plan.",
    ...
    formSubtitle:
      "We'll review the request and get back to you with a clear picture of costs and next steps.",
    name: "Full name",
    ...
    detailsPlaceholder:
      "What do you want to build or improve? The more detail, the more accurate our quote.",
      ...
      <main dir={dir} className="min-h-screen bg-background">
            ...
            <div className="mb-10 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
              <div>
                <span className="inline-flex items-center gap-1.5 text-secondary font-label-md text-label-md font-bold mb-2">
                  <span className="flex size-6 items-center justify-center rounded-full bg-secondary text-on-secondary text-xs font-bold">
                    1
                  </span>
                  {c.step1Label}
                </span>
                    ...
                    >
                      {isSelected && (
                        <span className="absolute top-4 end-4 flex size-6 items-center justify-center rounded-full bg-on-secondary/15">
                          <span
                            className="material-symbols-outlined text-[13px] text-on-secondary"
                            aria-hidden="true"
                          >
                            check
                          </span>
                      ...
                      <span
                        className={`material-symbols-outlined text-4xl mb-4 block ${isSelected ? "text-on-secondary" : "text-secondary"}`}
                        aria-hidden="true"
                      >
                        {service.icon}
                      </span>
                      <h3
                        className={`font-bold text-[16px] leading-snug mb-2 ${isSelected ? "text-on-secondary" : "text-primary"}`}
                      >
                        {service.title}
                      </h3>
                      <p
                        className={`text-caption leading-relaxed line-clamp-3 ${isSelected ? "text-on-secondary/75" : "text-on-surface-variant"}`}
                      >
                        {service.description}
                      </p>
                    </button>
        ...
        <section className="bg-surface-container-low py-16 md:py-24 px-margin-mobile md:px-margin-desktop">
          <Container clean>
            ...
            <h2 className="font-headline-xl text-headline-xl font-bold text-primary mb-10">
              {c.plansTitle}
            </h2>
            ...
            {!plansLoading && !plansError && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  {plans?.map((plan) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p
                          className={`font-label-md text-label-md mb-1 ${isSelected ? "text-on-primary/60" : "text-on-surface-variant"}`}
                        >
                          {plan.tier}
                        </p>
                        <h3
                          className={`font-bold text-[17px] mb-2 ${isSelected ? "text-on-primary" : "text-primary"}`}
                        >
                          {plan.name}
                        </h3>
                        <p
                          className={`text-caption leading-relaxed ${isSelected ? "text-on-primary/70" : "text-on-surface-variant"}`}
                        >
                          {plan.description}
                        </p>
                      </button>
                ...
                <div className="rounded-2xl border border-outline-variant/40 bg-white p-6 shadow-sm">
                  <h3 className="font-headline-sm text-headline-sm font-bold text-primary mb-5">
                    {c.budgetTitle}
                  </h3>
            ...
            {status === "success" ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[0.85fr_1.15fr] gap-10 items-start">
                <div>
                  ...
                  <h2 className="font-headline-xl text-headline-xl font-bold text-primary mt-1 mb-4">
                    {c.formTitle}
                  </h2>
...
function SuccessView({ locale, c }: { locale: string; c: (typeof copy)["ar"] }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-16">
      ...
      <p className="font-body-lg text-body-lg text-on-surface-variant max-w-md leading-relaxed mb-8">
        {c.success}
      </p>
      <Link
        to="/"
